...
async function getData(city) {
  let response = await fetch(
    `http://api.weatherapi.com/v1/forecast.json?key=3f666231b0c740d28a1204333210105&q=${city}&days=3`
  );
  let responseData = await response.json();
  let weatherDay1 = {
    dayName: getDayName(responseData.forecast.forecastday[0].date),
    date:
      getNumbersOfMonth(responseData.forecast.forecastday[0].date) +
      " " +
      getMonthName(responseData.forecast.forecastday[0].date),
